...
export const sumOrderFrame = (orders) => {
  let revenue = 0;
  let cost = 0;
...
export const buildTopProducts = (orders) => {
  ...
  for (const order of orders) {
    ...
    for (const item of items) {
      ...
      const key = item.productId
        ...
        : item._id
        ? String(item._id)
        : item.title || `product-${productTotals.size}`;
...
export const hydrateOrdersWithCost = async (rawOrders) => {
